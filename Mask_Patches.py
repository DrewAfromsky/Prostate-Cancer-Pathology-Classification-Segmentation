...
os.chdir('/home/ecbm4040/Final_Project/Gleason_masks_train')

if os.path.exists("/home/ecbm4040/Final_Project/Gleason_masks_train/Mask_Central_250x250") == False:
    central_dir = os.mkdir("/home/ecbm4040/Final_Project/Gleason_masks_train/Mask_Central_250x250")
else:
...
for file in sorted(glob.glob("*png")):
    ...
    nr, nc = a[1], a[2]
    os.chdir("/home/ecbm4040/Final_Project/Gleason_masks_train/Mask_Central_250x250")
    for i in range(nr):
        for j in range(nc):
            patch = tf.reshape(image_patches[0,i,j,], [ksize_rows, ksize_cols, 3]) # 750x750
            central = patch[250:-250, 250:-250,]
            c_file = tf.image.encode_png(central)
            c_writer = tf.io.write_file('Patch_{}_{}.png'.format(file[:-4],i*nc+j), c_file)
            sess.run(c_writer)            
